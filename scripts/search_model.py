...
Usage:
  search_model.py [--drop-ratings] [--out-name NAME] <dataset> <algorithm>
  search_model.py --inspect <file>
...
def job_template(data, var):
    ...
    return f'data/{data}/tuning/{ofn}-search.dvc', {
        'cmd': f'python -m scripts.search_model{opt} {data} {a_fn}',
        'wdir': '../../..',
        'deps': [
            {'path': 'scripts/search_model.py'},
            {'path': f'data/{data}/tuning/train-ratings.parquet'},
...
def setup(data, algo, implicit):
    ...
    _log.info('reading training ratings')
    train = pd.read_parquet(tdir / 'train-ratings.parquet')
